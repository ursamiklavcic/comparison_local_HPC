...
output_dir='/volumes/homehpc/storage/finished_projects/UrsaM/comparison_local_HPC/mothur_out_HPC'
number_threads='8'
...
set.current(processors=$number_threads)

# Stability file, 1st column name of sample, 2nd column forward read, 3rd column reverse read
make.file(inputdir="$input_dir", outputdir="$output_dir", type=gz, prefix=stability)
# Makes contigs
make.contigs(file=stability.files) 
set.current(inputdir="$output_dir")
